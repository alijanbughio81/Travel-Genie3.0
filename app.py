...
if results and inputs:
    ...
    with tab_summary:
        ...
        if not has_weather:
            st.info(
                "ℹ️ **Weather forecast not shown** — the trip falls outside Open-Meteo's "
                "16-day forecast window. Pick a start date within the next 16 days to see the weather."
            )
    ...
    with tab_itinerary:
        ...
        if not has_weather:
            st.caption(
                "ℹ️ Weather info not shown — your trip falls outside the "
